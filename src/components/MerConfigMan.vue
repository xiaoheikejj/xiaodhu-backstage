<template>
  <div id="MerConfigMan" style="width:100%;">
    <div class="main-hd">
      <div class="hd-title"><div class="icon-return" @click="returnRouter"></div>代理商权限配置</div>
    ...
    <el-dialog class="dialog checked" title="新增权限角色" :visible.sync="newAdd" width="400px" @open="aa">
      <el-form :model="newAddForm">
        ...
        <el-form-item label="角色描述：" style="margin-bottom:6px;">
          <el-input v-model="newAddForm.desc" style="width:256px;" clearable></el-input>
        </el-form-item>
    ...
    <!-- 修改-弹窗 -->
    <el-dialog class="dialog checked" title="代理商权限配置" :visible.sync="edit" width="400px">
  ...
  methods: {
    ...
    lookTree: function() {
        ...
        this.$nextTick(function() {
            var params = {
                agentPackSeq: that.row.agentPackSeq
            };
            Ajax('/agentauth/getAgentAuthByPackage', 'post', params).then(res => {
                res.map(function(currentValue, index) {
                    if (currentValue.agentAuthParentSeq != 0) {
                        that.editChecked.push(currentValue.agentAuthSeq);
                    }
                })
                //可选权限模块
                Ajax("/agentauth/getAuthList", "get").then(res => {
                    //禁用
                    res.forEach(function(ele, index) {
                        ele.disabled = true;
                        ele.tblAgentAAuthList.forEach(function(e, i) {
                            e.disabled = true;
                        })
                    })
                    that.treeList = res;
    ...
    tableInit() {
      ...
      params.append('agentPackName', this.topBtnGroupForm.name);
      this.$axios.post(baseUrl + "/agentpackage/searchAgentPackageList", params).then(res => {
    ...
    editConfirm() {
      ...
      arr.forEach(function(ele, index) {
        resourceSeq.push(ele.agentAuthSeq);
      });
      if (resourceSeq.length > 0) {
        var sendObj = {
            agentPackSeq: that.row.agentPackSeq,
            agentAuthSeq: resourceSeq
        }
         $.ajax({
             url: baseUrl + '/agentpackage/editPackageAuth',
             type: 'post',
             dataType: 'json',
             xhrFields: {
    ...
    newAddConfirm() {
      ...
      var packageDesc = this.newAddForm.desc;
      arr.forEach(function(ele, index) {
        resourceSeq.push(ele.agentAuthSeq);
      });
      if (packageName) {
        if (resourceSeq.length > 0) {
            var sendObj = {
                agentPackName: packageName,
                agentAuthSeq: resourceSeq,
                agentPackDesc: packageDesc
            };
            $.ajax({
                url: baseUrl + '/agentpackage/addAgentPackage',
                type: 'post',
                dataType: 'json',
                xhrFields: {
                       withCredentials: true
